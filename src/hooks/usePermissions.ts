...
const getPlatformSettingsPath = (t: TFunction): string =>
  t("hooks.permissions.paths.windowsMicrophone");

const getPlatformPrivacyPath = (t: TFunction): string =>
  t("hooks.permissions.paths.windowsMicrophone");

const getPlatform = (): "win32" => "win32";
...
export const usePermissions = (
  ...
  // On Windows, accessibility is always granted (PowerShell SendKeys is built-in).
  const accessibilityTroubleshooting = false;
  ...
  const checkPasteToolsAvailability = useCallback(async (): Promise<PasteToolsResult | null> => {
    ...
    try {
      if (window.electronAPI?.checkPasteTools) {
        ...
        // Windows: PowerShell SendKeys is always available — auto-grant accessibility.
  ...
  const requestAccessibilityPermission = useCallback(async () => {
    // Windows: PowerShell SendKeys is always available.
    setAccessibilityPermissionGranted(true);
  }, [setAccessibilityPermissionGranted]);
