...
- **STT pipeline (mic → engine → DB → paste)**: hotkey/click/auto-meeting → `audioManager` MediaStream → IPC `dictation-realtime-start` or `transcribe-local-whisper`/`transcribe-local-parakeet` (Blob) → main process → `whisperManager.transcribe()` / `parakeetManager.transcribe()` (lazy engine start) → optional `transformManager` (LLM cleanup) → `db-save-transcription` → `paste-text` IPC → `clipboardManager` writes via `electron.clipboard.writeText` then simulates paste through platform helper (`windows-fast-paste`, `macos-fast-paste.swift`, `linux-fast-paste`, or `ydotool`/`xdotool`/`wl-clipboard`). Result also surfaces in `TranscriptionPreviewOverlay.tsx` for in-app review/edit.
...
- `scripts/` — referenced by `package.json` lifecycle hooks (compile:mac-icon, afterPack, download:*, build-*). **[Caveat: in this checkout the directory is empty; agent must not promise to run any `scripts/*` file without confirming presence.]**
...
- `electron-builder.json` + `electron-builder.unsigned-win.json` — packaging config; `nix/` for Nix packaging.
...
- Build packaged app: `npm run build` — `cd src && vite build && cd .. && electron-builder`. Platform-specific: `npm run build:mac` (`:arm64`/`:x64`), `npm run build:win`, `npm run build:linux` (with `appimage`/`deb`/`rpm`/`tar`). Each prebuild chain compiles native helpers + downloads engine binaries + (on macOS) compiles the icon.
...
- Lifecycle hooks (do NOT run these manually): `prestart`, `predev`, `predev:main`, `prebuild` (compile:native + download:meeting-aec-helper), `prebuild:mac|win|linux` (prebuild + platform extras), `prepack`, `predist`, `postinstall` (`electron-builder install-app-deps`).
...
- `src/helpers/llamaServer.js` — llama.cpp OpenAI-compat server. `PORT_RANGE 8221-8240`, `DEFAULT_IDLE_TIMEOUT_MS = 5*60*1000` (overridable via `llmIdleTimeoutMs`). `getBackendChain(gpuMode)` → CUDA/Vulkan/CPU/Metal. Retries once with capability flags stripped.
...
- `electron-builder.json` + `electron-builder.unsigned-win.json` — packaging (`appId: com.gizmolabs.ektoswhispr`, `afterPack: scripts/afterPack.js`, asarUnpack for native modules, mac/win/linux targets, GitHub release publish).
