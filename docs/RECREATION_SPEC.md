...
13 workflows: `tests.yml` (PR, `npm ci --ignore-scripts && npm test`), `lockfile-lint.yml`, `codeql.yml` (segunda-feira + push/PR), `build-and-notarize.yml` (push/PR, build Windows assinado via Azure Trusted Signing em push / não-assinado em PR; desde a otimização de CI, a divergência PR-vs-push também é de escopo do target, não só de assinatura — PR compila apenas o target `--dir` (unpacked, sem NSIS/portable), enquanto push/`workflow_dispatch` continua com o build completo `nsis`+`portable` assinado), `release.yml` (tag `v*.*.*`, publica release oficial), `auto-release.yml` (bump patch automático em push main), 6 workflows de binário nativo individual (`build-windows-key-listener.yml`, etc. — `workflow_dispatch`, compilam e publicam tag própria `{componente}-v*`), `update-nix.yml` (atualiza `nix/package.nix`). (`build-windows-mic-listener.yml` existia para o binário de detecção automática de reuniões; removido junto com ela — ver §3.4.)
